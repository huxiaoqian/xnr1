...
def get_xnr_keywords(xnr_user_no,datetime_list):
    ...
    for date_name in datetime_list:
        xnr_flow_text_index_name = fb_xnr_flow_text_index_name_pre + date_name
    ...
    #添加容错
    #try:
    es_results = es_xnr.search(index = xnr_flow_text_index_name_list,doc_type = fb_xnr_flow_text_index_type,body=query_body)['hits']['hits']
    keywords_list = []
...
def detect_by_seed_users(seed_users):
    ...
    if retweet_mark == 1:
        ...
        be_retweet_index_name = fb_be_retweet_index_name_pre + str(db_number)
        #mget retwet
        '''
        try:
        ...
        #mget be_retweet
        try:
            be_retweet_result = es_xnr.mget(index=fb_be_retweet_index_name_pre, doc_type=fb_be_retweet_index_type, \
                                                body={'ids':seed_users} ,_source=True)['docs']
        except:
            be_retweet_result = []
    '''
