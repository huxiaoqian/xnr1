...
from global_utils import twitter_flow_text_index_name_pre,twitter_flow_text_index_type,es_xnr
...
def get_sensitive_info(timestamp,mid):
    index_name = twitter_flow_text_index_name_pre + ts2datetime(timestamp)
    try:
        item_result = es_xnr.get(index=index_name,doc_type=twitter_flow_text_index_type,id=mid)['_source']
