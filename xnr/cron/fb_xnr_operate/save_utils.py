...
from global_utils import fb_hot_content_recommend_results_index_name,\
                            ...
                            fb_hot_keyword_task_index_name,fb_hot_keyword_task_index_type,\
...
def save_content_recommendation_results(xnr_user_no,mid,task_id,content_results):
    ...
    print '123123123'
    es.index(index=fb_hot_content_recommend_results_index_name,\
        doc_type=fb_hot_content_recommend_results_index_type,\
    ...
    es.update(index=fb_hot_keyword_task_index_name,doc_type=fb_hot_keyword_task_index_type,\
...
def save_subopnion_results(xnr_user_no,mid,task_id,sub_opinion_results):
    ...
    item_exist['xnr_user_no'] = xnr_user_no
    item_exist['mid'] = mid
    item_exist['subopinion_fb'] = json.dumps(sub_opinion_results)
    #print 'sub_opinion_results::',sub_opinion_results

    es.index(index=fb_hot_subopinion_results_index_name,doc_type=fb_hot_subopinion_results_index_type,\
                id=task_id,body=item_exist)

    item_task = dict() 
    item_task['compute_status'] = 2  ## 保存子观点结果
    print 'fb_hot_keyword_task_index_name...',fb_hot_keyword_task_index_name
