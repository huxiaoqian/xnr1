#-*- coding: utf-8 -*-
'''
facebook_xnr warming function
'''
import os
import json
import time
from xnr.time_utils import ts2datetime,datetime2ts,get_day_flow_text_index_list
from xnr.global_utils import es_xnr,facebook_user_warning_index_name_pre,facebook_user_warning_index_type,\
                             facebook_xnr_fans_followers_index_name,facebook_xnr_fans_followers_index_type,\
                             es_flow_text,flow_text_index_name_pre,flow_text_index_type,\
                             facebook_feedback_follow_index_name,facebook_feedback_follow_index_type,\
                             facebook_xnr_index_name,facebook_xnr_index_type,\
                             facebook_speech_warning_index_name_pre,facebook_speech_warning_index_type

from xnr.parameter import HOT_facebook_NUM,MAX_VALUE,MAX_SEARCH_SIZE,DAY,FLOW_TEXT_START_DATE,REMIND_DAY
...
def lookup_xnr_fans_followers(user_id,lookup_type):
    try:
        xnr_result=es_xnr.get(index=facebook_xnr_fans_followers_index_name,doc_type=facebook_xnr_fans_followers_index_type,id=user_id)['_source']
...
def lookup_xnr_uid(xnr_user_no):
    try:
        xnr_result=es_xnr.get(index=facebook_xnr_index_name,doc_type=facebook_xnr_index_type,id=xnr_user_no)['_source']
...
def lookup_today_personal_warming(xnr_user_no,start_time,end_time):
    ...
    for user in top_userlist:
        ...
        try:
            user_result=es_xnr.get(index=facebook_feedback_follow_index_name,doc_type=facebook_feedback_follow_index_type,id=user_lookup_id)['_source']
