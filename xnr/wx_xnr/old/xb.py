...
class MyBot(Bot):
    #继承Bot()，以便加入更多属性
    def __init__(self, bot_id, temp_path, es_groupmsg, if_enable_puid=None, if_cache_path=None, console_qr=False, qr_path=None, if_qr_callback=None, login_callback=None, logout_callback=None):
        self.bot_id = bot_id
        ...
        wxxb = ensure_one(self.mps().search(u'小冰'))
        self.wxxb = wxxb
        @self.register(wxxb)
...
def init_es():
    es_groupmsg = WX_XNR_ES(host=config['es_host'], index_name='wxxb', doc_type='msg')
    es_groupmsg.create_index()
    es_groupmsg.put_mapping(doc_type=es_groupmsg.doc_type, mapping=config['wx_xnr_groupmsg_mapping'])
    return es_groupmsg

def push_msg(bot_id, msg):
    try:
        bot = WX_XNR_Bot[bot_id]
        u = bot.wxxb
        try:
            mm = bot.messages.search(sender=bot.wxxb)[-1]
            if mm.id not in bot.bf:
                bot.bf.add(mm.id)
        except:
            pass
        m = u.send(msg)
        ...
        while True:
            try:
                #使用布隆过滤器根据msgid检测此条消息是否已发送过
                mmm = bot.messages.search(sender=bot.wxxb)[-1]
                if mmm.id not in bot.bf:
                    bot.bf.add(mmm.id)
                    break
            except Exception,e:
                pass
...
def restart_bot(bot_id):
    try:
        ...
        pass
    try:
        bot = MyBot(bot_id=bot_id, temp_path=config['temp_path'], es_groupmsg=es_groupmsg, if_enable_puid=True, if_cache_path=True, console_qr=True, if_qr_callback=None)
...
def tcplink(conn, addr):
    ...
    if data:
        ...
        opt = data['opt']
        if opt == 'pushmsg':
            result = push_msg(bot_id=data['bot_id'], msg=data['m'])
        elif opt == 'restartbot':
...
def main():
    ...
    bot = MyBot(bot_id='xb', temp_path=temp_path, es_groupmsg=es_groupmsg, if_enable_puid=True, if_cache_path=True, console_qr=True, if_qr_callback=None)
    WX_XNR_Bot['xb'] = bot
